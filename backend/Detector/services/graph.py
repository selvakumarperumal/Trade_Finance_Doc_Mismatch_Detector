"""The Pydantic Graph pipeline.
    ...
    start -> ingest -> (fan out, one branch per document)
                          ocr -> classify -> route by type -> extract_<family>
                       (join)
                    -> reconcile -> end

The fan-out is a `map` fork, so every document is read, classified and extracted at the
same time; the `collect` join waits for all of them and hands reconciliation the full
set. Routing is a `Decision` built from the same table as the extraction steps, so a
branch can never point at a step that does not exist.

One rule runs through every step here: **a problem with one document is a finding, not a
failed case.** A scan that will not read, text that cannot be classified, an extractor
that errors — each travels on as an `ExtractedDocument` carrying its reason, and the
other documents are still checked. Only two things stop a whole case: the model budget
being spent (`UsageLimitExceeded`) and the run being cancelled (`RunCancelled`). Every
`except` below re-raises those two and contains everything else.
"""
...
"""Most severe first, which is the order an examiner wants to read findings in."""
...
# --- what the models are shown ----------------------------------------------


def _document_prompt(raw: RawDocument) -> str:
    """Wrap one document's text so the model can see its identity and its boundaries.

    The uploader asserts nothing about what a document is: they upload a file and the
    classifier decides. So the only evidence here is the text itself, fenced off in
    `<document_text>` tags, plus enough identity for the model to refer to it.
    """
    return (
...
def _reconciliation_prompt(state: CaseState, documents: Sequence[ExtractedDocument]) -> str:
    ...
    if state.presented_on is not None:
        presentation_date = f'Date of presentation: {state.presented_on.isoformat()}'
    else:
        presentation_date = (
...
async def ingest(ctx: StepContext[CaseState, DetectorDeps, CaseInput]) -> list[RawDocument]:
    ...
    ctx.state.record(
        'ingest', f'accepted {len(case.documents)} document(s), {to_read} to read with OCR'
    )
...
async def ocr(ctx: StepContext[CaseState, DetectorDeps, RawDocument]) -> RawDocument:
    ...
    Every way of not getting text ends the same way — `_unread` sends the document on
    with `ocr_error` set, `classify` then routes it to `unknown` without spending a model
    call on it, and it reaches the examiner as a finding.
    """
    raw = ctx.inputs
    ...
    return raw.model_copy(update={'text': read.text, 'page_count': read.page_count, 'content': None})


def _unread(
    ctx: StepContext[CaseState, DetectorDeps, RawDocument], raw: RawDocument, reason: str
) -> RawDocument:
    """Send a document on with no text, and say why."""
    ctx.state.record('ocr', reason, document_id=raw.document_id, document_type=DocumentType.UNKNOWN)
    return raw.model_copy(update={'ocr_error': reason, 'content': None})


@builder.step
async def classify(ctx: StepContext[CaseState, DetectorDeps, RawDocument]) -> RoutedDocument:
    ...
    have to second-guess. That covers three cases: no text to read, the call failing, and
    a guess the classifier is not sure enough about.
    """
    raw = ctx.inputs
    deps = ctx.deps
    ...
    if raw.needs_ocr:
        ...
        return _give_up(ctx, raw, f'skipped: {reason}', reason)
    ...
    try:
        ...
        return _give_up(ctx, raw, reason, reason)
    ...
    return RoutedDocument(
        raw=raw, classification=classification, usage=TokenUsage.from_run_usage(result.usage)
    )
...
def _give_up(
    ...
    """Record why this document was not classified, and route it to `unknown`.

    No usage to fold in: either no model call was made, or the one that was made failed.
    """
    ctx.state.record(
        'classify', message, document_id=raw.document_id, document_type=DocumentType.UNKNOWN
    )
    return RoutedDocument(
        raw=raw,
...
def _extracted(
    ...
    *,
    ...
    """One document's finished branch, however it ended.

    Every path out of extraction builds one of these, and the fields carried down from
    OCR and classification are the same on all of them — only the outcome differs.
    """
    return ExtractedDocument(
        ...
        payload=payload,
        error=error,
        usage=routed.usage if usage is None else usage,
    )
    ...
    async def extract(
        ...
        try:
            result = await ctx.deps.agents.extractor(document_type).run(
                _document_prompt(routed.raw), usage_limits=ctx.deps.usage_limits
            )
        except (UsageLimitExceeded, RunCancelled):
            raise  # Case-level: the budget is spent, or the whole run is going away.
            ...
            ctx.state.record(
                'extract', error, document_id=routed.raw.document_id, document_type=document_type
            )
            return _extracted(routed, document_type, error=error)
        ...
        return _extracted(
            ...
            usage=routed.usage + TokenUsage.from_run_usage(result.usage),
        )
...
entry there and a prompt, and its step and its route appear on their own.
"""
...
async def skip_unclassified(
    ...
    that the pipeline could not read.
    """
    routed = ctx.inputs
    ctx.state.record(
        'skip',
...
# --- turning the model's report into the final one ---------------------------


def _derive_status(mismatches: Sequence[Mismatch]) -> CaseStatus:
    """Map findings to a verdict.

    The prompt asks the model for this too, but the mapping is a rule, not a judgement,
    so the rule wins: a report with a critical finding is blocked whatever the model
    wrote in `status`.
    """
...
def _finalise_report(
    ...
    """Sort the findings, add one for any unreadable document, and apply the status rule.

    The added finding is why a presentation containing a scan nobody could read can never
    come back `clean`.
    """
    ...
    mismatches.sort(key=lambda mismatch: _SEVERITY_ORDER[mismatch.severity])
...
def _warning_report(
    *,
    ...
    """A `needs_review` verdict carrying a single warning.

    The shape both stand-in reports below take. Never `clean`: nothing was compared, and
    a presentation nobody checked is exactly the one an examiner must look at by hand.
    """
...
def _no_evidence_report(documents: Sequence[ExtractedDocument]) -> ReconciliationReport:
    ...
    detail = (
        ...
        if not documents
        else f'None of the {len(documents)} presented document(s) could be read into '
        'structured fields.'
    )
    return _warning_report(
